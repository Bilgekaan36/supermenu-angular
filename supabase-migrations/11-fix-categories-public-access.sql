...
-- Keep other policies for authenticated admin access
CREATE POLICY IF NOT EXISTS "categories_insert_by_restaurant"
  ON public.categories
  FOR INSERT
  TO authenticated
  WITH CHECK (restaurant_slug = 'eiscafe-remi');

CREATE POLICY IF NOT EXISTS "categories_update_by_restaurant"
  ON public.categories
  FOR UPDATE
  TO authenticated
...
CREATE POLICY IF NOT EXISTS "categories_delete_by_restaurant"
  ON public.categories
  FOR DELETE
  TO authenticated
  USING (restaurant_slug = 'eiscafe-remi');
